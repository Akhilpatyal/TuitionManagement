...
const [generatedQuiz, setGeneratedQuiz] = useState<GeneratedQuizType | null>(null);
  ...
  const changeQuestionType = (idx: number, type: string) => {
    if (type === 'MCQ') updateQuestion(idx, { type, options: ['', '', '', ''], correctAnswer: '0' });
    else if (type === 'TRUE_FALSE') updateQuestion(idx, { type, options: undefined, correctAnswer: 'true' });
  ...
  const addQuestion = () => setManualQuestions((prev) => [...prev, blankQuestion()]);
  const removeQuestion = (idx: number) =>
    setManualQuestions((prev) => (prev.length > 1 ? prev.filter((_, i) => i !== idx) : prev));
  ...
  const detectSubject = (fileName: string): string => {
    ...
    if (lower.includes('physics') || lower.includes('electro') || lower.includes('optic') || lower.includes('thermo') || lower.includes('mechanic')) return 'Physics';
    if (lower.includes('chem') || lower.includes('organic') || lower.includes('kinetic') || lower.includes('reaction') || lower.includes('mole')) return 'Chemistry';
    if (lower.includes('math') || lower.includes('calc') || lower.includes('algebra') || lower.includes('limit') || lower.includes('probab') || lower.includes('statistic')) return 'Mathematics';
  ...
  const handlePublishQuiz = () => {
    ...
    alert('AI-generated quiz successfully published and dispatched to student dashboards.');
  };
    ...
    <div className="space-y-8">
        ...
        <div className="lg:col-span-6 glass-card rounded-3xl p-6 flex flex-col justify-between">
          <div className="space-y-5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-md font-bold text-slate-200">AI Quiz Generation Agent</h3>
            </div>
            
            {/* Document upload zone */}
            <div>
              <label className="block text-[10px] uppercase font-mono text-slate-500 mb-2">Upload Reference Material</label>
                ...
                >
                  {isUploading ? (
                    <div className="flex flex-col items-center gap-3 py-2">
                      ...
                      <p className="text-xs font-mono text-indigo-400 animate-pulse">Uploading file to server...</p>
                    </div>
                  ) : (
                    <>
                      <UploadCloud className={`w-8 h-8 mx-auto mb-2 transition-colors ${isDragging ? 'text-indigo-400' : 'text-slate-600'}`} />
                      <p className="text-xs text-slate-400 font-medium">
                        {isDragging ? 'Drop file to upload' : 'Drag & drop a PDF here, or click to browse'}
                      </p>
                      <p className="text-[10px] text-slate-600 font-mono mt-1">Supports PDF, DOC, DOCX, TXT, PNG, JPG</p>
                    </>
                  )}
                  ...
                  {!isUploading && !isDragging && (
                    <div className="flex flex-wrap justify-center gap-2 mt-4" onClick={(e) => e.stopPropagation()}>
                      <span className="text-[9px] text-slate-600 font-mono uppercase w-full mb-1">Or use demo templates:</span>
                      <button
                        onClick={() => handleSimulateUpload('Electromagnetism_Theory.pdf', 'Physics')}
                      ...
                      <button
                        onClick={() => handleSimulateUpload('Organic_Chemistry_Compounds.pdf', 'Chemistry')}
                        className="px-2.5 py-1 rounded bg-slate-900 border border-slate-800 text-[10px] text-slate-400 hover:text-white transition-colors cursor-pointer"
                      >
                        + Organic_Ch2.pdf
                      </button>
                      <button
                        onClick={() => handleSimulateUpload('Limits_And_Integrals.pdf', 'Mathematics')}
                        className="px-2.5 py-1 rounded bg-slate-900 border border-slate-800 text-[10px] text-slate-400 hover:text-white transition-colors cursor-pointer"
                      >
                        + Calc_Limits.pdf
                      </button>
                ...
                <div className="flex items-center justify-between p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/25">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-white truncate max-w-[200px]">{uploadedFile}</p>
                      <p className="text-[10px] text-indigo-400 font-mono">Reference Synced ✓</p>
                    </div>
                  </div>
                  <button
                    onClick={() => { setUploadedFile(null); setGeneratedQuiz(null); }}
            ...
            <div className="grid grid-cols-2 gap-4 text-xs text-slate-300">
              <div>
                <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1.5">Difficulty Profile</label>
                <select
                  value={difficulty}
              ...
              <div>
                <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1.5">Number of Questions</label>
                <input
                  type="number"
                  min={1}
                  max={50}
                  value={numQuestions}
                  onChange={(e) => setNumQuestions(Math.min(Math.max(parseInt(e.target.value, 10) || 1, 1), 50))}
                  className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-900 focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1.5">Assign to Student (Optional)</label>
                <select
                ...
                >
                  ...
                  {students.map(s => (
                    <option key={s.id} value={s.id}>{s.name} ({s.accuracyPct}% Acc)</option>
                  ))}
                </select>
              </div>
            </div>
        ...
        <div className="lg:col-span-6 glass-card rounded-3xl p-6 flex flex-col justify-between min-h-[350px]">
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-md font-bold text-slate-200">AI Compiler Output</h3>
            </div>
            ...
            {isGenerating && (
              <div className="h-64 flex flex-col items-center justify-center text-center space-y-4">
                ...
                {genStep === 1 && (
                  <p className="text-xs font-mono text-indigo-400 animate-pulse">Reading document & extracting concepts...</p>
                )}
                {genStep === 2 && (
                  <p className="text-xs font-mono text-purple-400 animate-pulse">Reviewing student profiles & setting difficulty...</p>
                )}
                {genStep === 3 && (
                  <p className="text-xs font-mono text-cyan-400 animate-pulse">Authoring unique questions & explanations...</p>
                )}
              </div>
            )}
            ...
            {!isGenerating && !generatedQuiz && (
              <div className="h-64 flex flex-col items-center justify-center text-center p-6 border border-dashed border-slate-900 rounded-2xl">
                ...
                <p className="text-xs text-slate-500 leading-relaxed font-mono">
                  Wait state. Configure details and execute the AI generator to output live preview sheets.
                </p>
              </div>
            )}
            ...
            {!isGenerating && generatedQuiz && (
              <div className="space-y-4 max-h-[380px] overflow-y-auto pr-1">
                <div className="p-3.5 rounded-2xl bg-indigo-500/5 border border-indigo-500/10">
                  ...
                  <p className="text-[10px] text-slate-400 leading-relaxed mt-1">{generatedQuiz.description}</p>
                ...
                {generatedQuiz.questions.map((q: any, idx: number) => (
                  <div key={idx} className="p-4 rounded-2xl bg-slate-950/40 border border-slate-900 space-y-2">
                    <span className="text-[9px] font-mono font-bold text-purple-400 uppercase">Question {idx + 1} ({q.type})</span>
                    ...
                    {q.options && (
                      <ul className="space-y-1 pl-2 text-[11px] text-slate-400 list-disc">
                        {q.options.map((opt: string, oIdx: number) => (
                          <li key={oIdx} className={oIdx.toString() === q.correctAnswer ? 'text-emerald-400 font-semibold list-none flex items-center gap-1.5' : ''}>
                            {oIdx.toString() === q.correctAnswer && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />}
                            {opt}
                          </li>
                        ))}
                      </ul>
                    ...
                    {!q.options && (
                      <p className="text-[10px] text-emerald-400 italic">Correct Answer: {q.correctAnswer}</p>
                    )}
                    <p className="text-[10px] text-slate-500 leading-relaxed bg-white/[0.01] p-2 rounded border border-white/[0.02]">
                      <strong className="text-slate-400 font-mono">Explanation:</strong> {q.explanation}
                    </p>
                  </div>
                ))}
              </div>
      ...
      <div className="glass-card rounded-3xl p-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-md font-bold text-slate-200">Manual Quiz Builder</h3>
        </div>
        <p className="text-[11px] text-slate-500 mb-6">No AI needed — write your own questions, options, and answers, then assign to a batch or student.</p>

        {/* Quiz meta */}
        <div className="space-y-4">
          <div>
            <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1.5">Quiz Title</label>
            <input
              type="text"
              value={manualTitle}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs text-slate-300">
            ...
            <div>
              <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1.5">Assign to Batch</label>
              <select
                value={manualBatch}
            ...
            <div>
              <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1.5">Duration (Mins)</label>
              <input
                type="number"
                min={1}
                value={manualDuration}
                onChange={(e) => setManualDuration(Math.max(parseInt(e.target.value, 10) || 1, 1))}
                className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-900 focus:outline-none"
              />
            </div>
            <div>
              <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1.5">Assign Student (Optional)</label>
              <select
        ...
        <div className="mt-6 space-y-4">
          {manualQuestions.map((q, idx) => (
            <div key={idx} className="p-4 rounded-2xl bg-slate-900/30 border border-slate-900/80 space-y-3">
              ...
              {q.type === 'MCQ' && q.options && (
                <div className="space-y-2">
                  <span className="text-[9px] font-mono uppercase text-slate-600">Tap the circle to mark the correct option</span>
                  {q.options.map((opt, oIdx) => {
                    ...
                    return (
                      <div key={oIdx} className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => updateQuestion(idx, { correctAnswer: oIdx.toString() })}
                          title="Mark correct"
                        ...
                        <input
                          ...
                          className={`flex-1 px-3 py-1.5 rounded-lg bg-slate-950 border text-xs text-slate-200 focus:outline-none ${
                            isCorrect ? 'border-emerald-500/30' : 'border-slate-900 focus:border-indigo-500'
                          }`}
                        />
                      </div>
                    );
              ...
              {q.type === 'SHORT_ANSWER' && (
                <div>
                  <label className="block text-[9px] font-mono uppercase text-slate-600 mb-1">Correct Answer</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-[9px] font-mono uppercase text-slate-600 mb-1">Explanation (Optional)</label>
                <textarea
                  value={q.explanation}
          ...
          >
            {isSavingManual ? (
              ...
              <>
                ...
                <span>Create &amp; Assign Quiz ({manualQuestions.length} {manualQuestions.length === 1 ? 'Question' : 'Questions'})</span>
              </>
            )}
          </button>
        </div>
            ...
            >
              <div className="flex gap-4 items-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">{quiz.description}</p>
                  ...
                  <div className="flex flex-wrap gap-4 mt-2.5 text-[10px] font-mono text-slate-500 uppercase font-bold">
                   ...
                   <span>Questions: {Array.isArray(quiz.questions) ? quiz.questions.length : 0}</span>
